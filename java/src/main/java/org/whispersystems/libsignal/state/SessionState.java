...
public class SessionState {
    ...
    public SessionState() {
        ...
        try {
            ...
            SessionStructure.RatchetDynamicMulticastStructure rdms = SessionStructure.RatchetDynamicMulticastStructure.newBuilder()
                    .addAllEphemeralPublicKey(ByteString.copyFrom(ephemeralRDMKeyPair.getPublic().getEncoded()))
                    .setMacKey(ByteString.copyFrom(macKey))
                    .setOwnEphemeralPublicKey(ByteString.copyFrom(ephemeralRDMKeyPair.getPublic().getEncoded()))
                    .setOwnEphemeralSecretKey(ByteString.copyFrom(ephemeralRDMKeyPair.getPrivate().getEncoded()))
                    .build();
            this.sessionStructure = SessionStructure.newBuilder()
                    .setRatchetDynamicMulticastStructure(rdms)
                    .build();
    ...
    public List<ByteString> getAllEphemeralPublicKey() {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        return new ArrayList<>(rdms.getAllEphemeralPublicKeyList());
    }

    public ByteString getOwnEphemeralSecretKey() {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        return rdms.getOwnEphemeralSecretKey();
    }

    public ByteString getOwnEphemeralPublicKey() {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        return rdms.getOwnEphemeralPublicKey();
    }
    ...
    public void updateAllEphemeralPublicKey(PublicKey pk) {
        SessionStructure.RatchetDynamicMulticastStructure build = this.sessionStructure.getRatchetDynamicMulticastStructure().toBuilder().addAllEphemeralPublicKey(ByteString.copyFrom(pk.getEncoded())).build();
        sessionStructure = sessionStructure.toBuilder().setRatchetDynamicMulticastStructure(build).build();
    }

    public byte[] getMacKey() {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        return rdms.getMacKey().toByteArray();
    }

    public byte[] updateRDM(PublicKey pk) {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        List<ByteString> allEphemeralPublicKeyList = rdms.getAllEphemeralPublicKeyList();
        allEphemeralPublicKeyList.add(ByteString.copyFrom(pk.getEncoded()));
        ByteString macKey = rdms.getMacKey();
        try {
            byte[] newMacKey = KeyGenerator.getInstance("HmacSHA256").generateKey().getEncoded();
            SessionStructure.RatchetDynamicMulticastStructure newrdms;
            newrdms = rdms.toBuilder()
                    .setMacKey(ByteString.copyFrom(newMacKey))
                    .addAllAllEphemeralPublicKey(allEphemeralPublicKeyList)
                    .build();
            this.sessionStructure = this.sessionStructure.toBuilder().setRatchetDynamicMulticastStructure(newrdms).build();

            // build tag with all ephemeral pub keys + new mac key with pre mac key
            Mac sha256_HMAC = Mac.getInstance("HmacSHA256");
            sha256_HMAC.init(new SecretKeySpec(macKey.toByteArray(), "HmacSHA256"));
            ...
            return sha256_HMAC.doFinal(data.toByteArray());
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
    ...
    public SessionStructure.RatchetDynamicMulticastStructure getRatchetDynamicMulticastStructure() {
        return sessionStructure.getRatchetDynamicMulticastStructure();
    }

    public void setRatchetDynamicMulticastStructure(SessionStructure.RatchetDynamicMulticastStructure rdms) {
        this.sessionStructure = this.sessionStructure.toBuilder().setRatchetDynamicMulticastStructure(rdms).build();
    }
    ...
    public Chain getSenderChain() { return sessionStructure.getSenderChain();}
    ...
    public void rdmFilter() {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        SessionStructure.RatchetDynamicMulticastStructure build = rdms.toBuilder().clearOwnEphemeralSecretKey().clearOwnEphemeralPublicKey().build();
        this.sessionStructure = SessionStructure.newBuilder()
                .setRatchetDynamicMulticastStructure(build)
                .build();
    }

    public void signalFilter() {
        SessionStructure.RatchetDynamicMulticastStructure rdms;
        rdms = SessionStructure.RatchetDynamicMulticastStructure.newBuilder().build();
        this.sessionStructure = this.sessionStructure.toBuilder().setRatchetDynamicMulticastStructure(rdms).build();
    }

    public void updateOwnpheralKeys(PrivateKey devicePrivateKey, PublicKey devicePublicKey) {
        SessionStructure.RatchetDynamicMulticastStructure rdms = this.sessionStructure.getRatchetDynamicMulticastStructure();
        SessionStructure.RatchetDynamicMulticastStructure build = rdms.toBuilder()
                .setOwnEphemeralSecretKey(ByteString.copyFrom(devicePrivateKey.getEncoded()))
                .setOwnEphemeralPublicKey(ByteString.copyFrom(devicePublicKey.getEncoded()))
                .build();
